...
router.use("/GetAllCompleteOrders", async (req, res) => {
  const memberSid = req.token.sid
  // console.log(memberSid);
  const sql = "SELECT o.`sid`, o.`shop_sid`, o.`deliver_sid`, o.`store_order_sid`, o.`deliver_order_sid`, o.`order_time`, o.`order_total`,o.`coupon_sid`,o.`sale`,o.`pay_method`,o.`deliver_fee`,o.`total_amount`,s.`name` shopName , s.`src` , do.`complete_time` ,d.`name` deliverName ,cc.`coupon_name`,de.`evaluation_score`  deliverScore ,se.`evaluation_score` shopScore FROM `orders` o LEFT JOIN `deliver_order` do ON o.`sid` = do.`order_sid` LEFT JOIN `deliver` d  ON d.`sid` = do.`deliver_sid` LEFT JOIN `shop` s ON s.`sid` = o.`shop_sid` LEFT JOIN `coupon_content` cc ON cc.`sid` = o.`coupon_sid` LEFT JOIN `shop_evaluation` se ON se.`order_sid` = o.`sid` LEFT JOIN `deliver_evaluation` de ON de.`order_sid` = o.`sid`  WHERE o.`member_sid` = ? AND o.`order_complete` = 1 "
...
router.use("/GetDropDetails", async (req, res) => {
